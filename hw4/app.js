...
db.query(
  "CREATE TABLE IF NOT EXISTS posts (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, title TEXT, body TEXT)"
);
db.query(
...
router
  ...
  .get("/post/:id", show)
  .post("/post", create)
  .get("/list/:user", listUserPosts); // Route to show posts for a specific user
...
async function show(ctx) {
  ...
  let posts = postQuery(
    `SELECT id, username, title, body FROM posts WHERE id=${pid}`
  );
  let post = posts[0];
