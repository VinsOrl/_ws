...
export function list(posts, user) {
  ...
  <p>${
    user == null
      ...
      : "Welcome " +
        user.username +
        ', You may <a href="/post/new">Create a Post</a> or <a href="/logout">Logout</a>!'
  }</p>
...
export function listUserPosts(posts, user) {
  let list = [];
  for (let post of posts) {
    list.push(`
    <li>
  ...
  <p>There are <strong>${posts.length}</strong> posts by ${user}!</p>
  <ul id="posts">
    ${list.join("\n")}
  </ul>
  `;
  return layout(`Posts by ${user}`, content);
}
